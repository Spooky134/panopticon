...
async def create_session(request: StreamingSessionCreateRequest,
    ...
    streaming_session_id, streaming_session_data = await streaming_session_lifecycle_service.create_session(**request.model_dump())
    return StreamingSessionResponse(streaming_session_id=streaming_session_id, **asdict(streaming_session_data))
...
async def get_session(request: StreamingSessionRequest,
                      streaming_session_lifecycle_service: StreamingLifecycleServiceDep):
    streaming_session_id, streaming_session_data = await streaming_session_lifecycle_service.read_session(**request.model_dump())
    return StreamingSessionResponse(streaming_session_id=streaming_session_id, **asdict(streaming_session_data))
